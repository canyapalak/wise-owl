...
import { CustomQuizResultProps } from "../types";
...
export default function CustomQuizResult({
  closeCustomQuizResult,
}: CustomQuizResultProps) {
  const { score } = useContext(ScoreContext);
  const { questionAmount } = useContext(CustomQuizContext);

  const getAnimalImage = (score: number) => {
    if (score >= 0 && score <= Math.floor(questionAmount * 0.2)) {
      return crow;
    } else if (
      score > Math.floor(questionAmount * 0.2) &&
      score <= Math.floor(questionAmount * 0.4)
    ) {
      return cat;
    } else if (
      score > Math.floor(questionAmount * 0.4) &&
      score <= Math.floor(questionAmount * 0.6)
    ) {
      return chicken;
    } else if (
      score > Math.floor(questionAmount * 0.6) &&
      score <= Math.floor(questionAmount * 0.8)
    ) {
      return fox;
    } else {
      return owl;
    ...
    if (score >= 0 && score <= Math.floor(questionAmount * 0.2)) {
      return (
        <p className="text-justify px-3">
          ...
          out of <span className="text-brick-default">{questionAmount}</span>{" "}
          and become the <span className="text-brick-default">Baby Crow</span>.
      ...
      return (
        <p className="text-justify px-3">
          ...
          answers out of{" "}
          <span className="text-brick-default">{questionAmount}</span>! Well,
      ...
      return (
        <p className="text-justify px-3">
          ...
          <span className="text-brick-default">{questionAmount}</span>! You're
          right in the middle, strutting around with the confidence of a chicken
          who knows how to navigate the barnyard without causing too much
          commotion. You might not be laying the golden eggs of knowledge just
          yet, but your feathers are looking quite average and that's egg-sactly
      ...
      return (
        <p className="text-justify px-3">
          ...
          questions out of{" "}
          <span className="text-brick-default">{questionAmount}</span> . With
      ...
      return (
        <p className="text-justify px-3">
          ...
          out of <span className="text-brick-default">{questionAmount}</span>.
          Your intellect shines like moonlight through the forest! Your
          knowledge is a vast, ancient library, and you've emerged as the
          luminary guardian of the quiz realm. Continue to spread your wings of
          wisdom, and illuminate the minds of those who seek knowledge, while
          you are soaring ever higher into the realm of enlightenment.
        </p>
      );
    }
  };
